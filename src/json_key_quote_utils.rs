...
pub fn json_add_key_quotes(json: &str, quote_type: Quotes) -> String {
    ...
    // `/` == `\/` in Regex101
    let single_quoted_string_val_regex = Regex::new(
    ...
    // `/` == `\/` in Regex101
    let double_quoted_string_val_regex = Regex::new(
        &(r#"(?P<prevchar_key>[^"'][\s]*)(?P<key>["#.to_string()
            + SUPPORTED_KEY_CHARS_REGEX_STR
            + r#"]*?[^"'])(?P<val>:\s*?"[\s\S]*?")"#),
    )
    ...
    // `/` == `\/` in Regex101
    let object_val_regex = Regex::new(
        &(r#"(?P<key>["#.to_string()
            + SUPPORTED_KEY_CHARS_REGEX_STR
            + r#"]*?[^"'])(?P<val>:\s*?[{\[])"#),
    )
    ...
    // `/` == `\/` in Regex101
    let number_val_regex = Regex::new(
    ...
    // `/` == `\/` in Regex101
    let null_bools_val_regex = Regex::new(
        &(r#"(?P<before>[\[,{]\s*?)(?P<key>["#.to_string()
            + SUPPORTED_KEY_CHARS_REGEX_STR
...
pub fn json_remove_key_quotes(json: &str) -> String {
    ...
    // `/` == `\/` in Regex101
    let single_quotes_regex = Regex::new(
        &(r#"(?P<before>[{\[,][\s]*)'(?P<key>["#.to_string()
            + SUPPORTED_KEY_CHARS_REGEX_STR
            + r#"]*?)'(?P<after>\s*?:)"#),
    )
    ...
    // `/` == `\/` in Regex101
    let double_quotes_regex = Regex::new(
        &(r#"(?P<before>[{\[,][\s]*)"(?P<key>["#.to_string()
            + SUPPORTED_KEY_CHARS_REGEX_STR
            + r#"]*?)"(?P<after>\s*?:)"#),
    )
...
pub fn json_escape_ctrlchars(json: &str) -> String {
    ...
    for _n in 0..2 {
        ...
        let singlequoted_string_key_regex = Regex::new(
            &(r#"(?P<prevchar_key>[^"'][\s]*)'(?P<key>["#.to_string()
                + SUPPORTED_KEY_CHARS_REGEX_STR
                + r#"]*?[^"'])'(?P<val>\s*?:\s*?'[\s\S]*?')"#),
        )
        ...
        for cap in singlequoted_string_key_regex.captures_iter(&new_json.clone()) {
            ...
            new_json = new_json.replacen(
                cap_match,
                &cap_match.replace("\t", "").replace("\t", "\\t"),
                1,
            );
        }
        ...
        let doublequoted_string_key_regex = Regex::new(
            &(r#"(?P<prevchar_key>[^"'][\s]*)'(?P<key>["#.to_string()
                + SUPPORTED_KEY_CHARS_REGEX_STR
                + r#"]*?[^"'])'(?P<val>\s*?:\s*?"[\s\S]*?")"#),
        )
        ...
        let object_key_regex = Regex::new(
            &(r#"'(?P<key>["#.to_string()
                + SUPPORTED_KEY_CHARS_REGEX_STR
                + r#"]*?[^"'])'(?P<val>\s*?:\s*?[{\[])"#),
        )
        ...
        let number_key_regex = Regex::new(
            &(r#"(?P<before>[\[,{]\s*?)'(?P<key>["#.to_string()
                + SUPPORTED_KEY_CHARS_REGEX_STR
                + r#"]*?[^"'])'(?P<after>\s*?:\s*?[\d\-\.])"#),
        )
        ...
        let null_boolean_key_regex = Regex::new(
            &(r#"(?P<before>[\[,{]\s*?)'(?P<key>["#.to_string()
                + SUPPORTED_KEY_CHARS_REGEX_STR
...
/// and remove unescaped ctrl-characters from JSON keys without keyquotes.
///
/// This method will unescape `newlines` and `tabs` in the JSON string values
/// and remove unescaped `newlines` and `tabs from the JSON keys without keyquotes.
///
/// # Arguments
///
/// * `json` - The JSON string.
...
"#, 1,
    ...
    fn test_json_unescape_ctrlchars_single_quoted_supported_characters() {
        ...
        let expected_key = supported_key_chars
            .replacen(
                "A", r#"A
"#, 1,
            )
